...
@section('content_header')
    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        <button style="margin: 10px;" class="btn btn-success"
            onclick="fnExportToExcel('xlsx','Planilla Final <?php echo $periodo->NombrePeriodo; ?>')" id="btnExportar">Exportar a Excel <i
...
@section('content')
    ...
    <table style="border: 2px solid #000;" id="tablaPlanilla"
        class="table table-bordered table-hover tablaPlanilla table-responsive">
        <thead>
            <tr>
            ...
            <tr>
                ...
                <th style="background-color: #C6C7BF;" class="th1 p-1 text-center align-middle"
                    colspan="{{ $num_estaciones + 14 }}">
                    TAREO
                </th>
                <th colspan="8"></th>
        ...
        <tbody>
            ...
            @foreach ($tareos as $item)
                ...
                @endphp
                {{--CONTENIDO DE REGISTRO DE CADA CONTRATO--}}
                <tr class="trTable">
                    <td>
                        {{-- DNI --}}
                        {{ $item->DNI }}
                    ...
                    <td style="background-color: #CEE3F5; font-weight:bolder;text-align: right;">
                        ...
                        @php
                            $sueldo_bruto = null;
                            // 1. Obtener los días de descanso programado en este período
                            $descansos_programados_obj = new \App\Http\Controllers\Planilla\PlanillaController();
                            $dias_descanso_programado = $descansos_programados_obj->DescansosProgramados(
                                $item->idContrato,
                                $dia_inicio,
                                $dia_fin,
                            );
                            $dias_faltas = $descansos_programados_obj->DiasDeFalta(
                                $item->idContrato,
                                $dia_inicio,
                                $dia_fin,
                            );
                    ...
                    <td>
                        ...
                        @php
                            $feriados_trabajados = round((($sueldo_base / $divisor_dias) * 2)    * $feriados_trabajados_dias, 2);
