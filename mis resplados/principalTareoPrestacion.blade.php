                            ...
                            @for ($date = $fechaInicial; $date <= $fechaFinal; $date->add($intervalo))
                                ...
                                @else
                                    <th class="text-center">
                                        <button class="btn btn-primary btnOpenModal" data-toggle="modal"
                                            data-target="#modalTareo"
                                            onclick="setIframeSrc('{{ url('tareos/ver-tareo/estacion/' . $idEstacion . '/' . $fechaActual->format('Y-m-d')) }}')">{{ $fechaActual->format('d') }}</button>
                                    </th>
                                @endif
                                @php
                    ...
                    <tbody>
                        @php
                            ...
                            $leyendasCondicion = [
                                ...
                                2 => 'T', // TARDANZA
                                3 => 'F', // FALTA
                                4 => 'DM', // DESCANSO MEDICO
                                5 => 'SL', // SIN LICENCIA
                                6 => 'CL', // CON LICENCIA
                                7 => 'DP', // DESCANSO PROGRAMADO
                                8 => 'FT', // FERIADO TRABAJADO
                                9 => 'V', // VACACIONES
                                10 => 'VIA', // VIAJE
                                11 => 'DT', // DESCANSO TRABAJADO
                                12 => 'HN', // HORARIO NOCTURNO
                                13 => 'PT', // PART TIME
                                14 => 'F', // FERIADO
                                15 => 'LP', // LICENCIA PATERNIDAD
                            ];
                        @endphp
                            ...
                            @if (!in_array($tareo->idContrato, $personasImpresas))
                                <tr>
                                    ...
                                    @for ($date = Carbon\Carbon::parse($fechaInicial); $date <= $fechaFinal; $date->add($intervalo))
                                            ...
                                            @if ($tareo->idContrato == $item->idContrato && $item->Fecha == $date)
                                                @php
                                                    $tiempo = tiempoTrabajado(
                                                        $item->HoraDeIngreso,
                                                        $item->HoraDeSalida,
                                                        $item->HoraDeInicioDeAlmuerzo,
                                                        $item->HoraDeFinDeAlmuerzo,
                                                    );
                                                    $tiempoSubTotal = tiempoTrabajadoTotal(
                                                        $tiempo,
                                                        $item->HoraDeIngreso,
                                                        $item->HoraDeSalida,
                                                        $item->HoraDeInicioDeAlmuerzo,
                                                ...
                                                @if ($esSabado)
                                                    @if (strtotime($tiempo) >= strtotime('05:30:00'))
                                                        <button type="button" class="btn btn-success btn-sm p-0 btn-tareo"
                                                            data-toggle="modal"
                                                            data-nombres="{{ strtoupper($tareo->Nombres . ' ' . $tareo->ApellidoPaterno . ' ' . $tareo->ApellidoMaterno) }}"
                                                            data-target="#tareoModal" data-tareo="{{ $item->idTareo }}"
                                                            data-fecha="{{ $item->Fecha->format('Y-m-d') }}">
                                                            {{ $tiempo }}
                                                        </button>
                                                    @else
                                                        <button type="button" class="btn btn-warning btn-sm p-0 btn-tareo"
                                                            data-toggle="modal"
                                                            data-nombres="{{ strtoupper($tareo->Nombres . ' ' . $tareo->ApellidoPaterno . ' ' . $tareo->ApellidoMaterno) }}"
                                                            data-target="#tareoModal" data-tareo="{{ $item->idTareo }}"
                                                            data-fecha="{{ $item->Fecha->format('Y-m-d') }}">
                                        ...
                                        @endforeach
                                        @if (!$fechaEncontrada)
                                            <button type="button" class="btn btn-outline-dark btn-sm p-1 btn-tareo"
                                                data-toggle="modal" data-target="#tareoModal"
                                                data-contrato="{{ $tareo->idContrato }}"
                                    ...
                                    <td class="bg-info">
                                        {{-- Mostrar dias tareadas --}}
                                        {{-- @php
                                    $totalDias = totalDiasTareados($tareos, $tareo->idContrato, $dia_inicio, $dia_fin);
                                    echo $totalDias;
                                @endphp --}}
                                    </td>
                                </tr>
