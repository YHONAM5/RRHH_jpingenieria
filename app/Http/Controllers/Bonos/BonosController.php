...
class BonosController extends Controller
{
    public function index()
    {
        ...
        return view('rrhh.bonos.index', compact('personas', 'estaciones', 'periodos'));
    }
    ...
    public function buscar_bonos(Request $request)
    {
        ...
        if ($bonos->count() > 0) {
            return view('rrhh.bonos.index', compact('personas', 'estaciones', 'periodos', 'bonos', 'idPeriodo'));
        } else {
            return view('rrhh.bonos.index', compact('personas', 'estaciones', 'periodos', 'idPeriodo'));
        }
    }
    ...
    public function registro_bono(Request $request)
    {
        try {
            ...
            if (empty($monto) && empty($bonoDeclarado)) {
                return response()->json(['error' => true, 'message' => 'Debes ingresar al menos un valor en Monto del Reintegro o Monto del Bono Declarado.']);
            }
            ...
            $bono->BonoDeclarado = $bonoDeclarado; // Asignar el bono declarado, si existe
            $bono->save();
    
            return response()->json(['success' => true, 'message' => 'Reintegro y/o Bono Declarado registrados correctamente']);
        } catch (\Exception $e) {
            return response()->json(['error' => true, 'message' => 'Error al registrar el reintegro: ' . $e->getMessage()]);
        }
    }
